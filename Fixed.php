...
<html lang="en">
<head>
  <title> Fixed Predictions - <?php echo $app_name; ?></title>
...
<div class="site-wrap">
  ...
  <header class="site-navbar py-4" role="banner">
      ...
      <div class="container">
        <div class="row align-items-center">    
          <div class="col-lg-5 mx-auto text-center">
            <h1 class="text-white">FIXED ODDS</h1>
   ...
   <div class="container">
    <div class="banner">
      PLEASE NOTE THAT HERE WE ONLY UPDATE YOU OUR PREVIOUS CORRECT SCORE MATCHES RESULTS WHICH IS BEING POSTED IN OUR VIP ROOM. THE SYSTEM AUTOMATICALLY TRANSFER OUT THE MATCHES FROM VIP ROOM AFTER THE WINNING FOR ALL USERS TO CONFIRM WHAT'S REALLY HAPPENING IN OUR VIP ROOM.
    </div>

    <div class="section-title">Past successful Fixed Matches</div>
    <?php
    $nextDay = date('l, F j, Y', strtotime('+1 day'));
    ?>
    <div class="next-match">Next Fixed Match scheduled for - <?php echo $nextDay; ?></div>
    ...
    // Total date count
    $totalDatesStmt = $dbh->query("SELECT COUNT(DISTINCT DATE(match_date)) AS total FROM predictions WHERE type = 'fixed'");
    $totalDates = $totalDatesStmt->fetchColumn();
    $totalPages = ceil($totalDates / $limit);

    // Get paginated dates
    $datesQuery = "SELECT DISTINCT DATE(match_date) AS match_date_only FROM predictions WHERE type = 'fixed' ORDER BY match_date_only DESC LIMIT $limit OFFSET $offset";
    ...
    if ($datesResult && count($datesResult) > 0) {
        ...
        foreach ($datesResult as $dateRow) {
            ...
            echo '<div class="vip-header">Past Fixed Matches - ' . $formattedDate . '</div>';
        ...
        echo '<div>No past fixed matches found.</div>';
    }
    ?>
  </div>
