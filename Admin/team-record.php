...
// Fetch team records with league info
$stmt = $dbh->prepare("
    SELECT teams.*, leagues.name AS league_name, leagues.country 
    FROM teams 
...
<html lang="en">
<head>
  <title>Team Records - <?= htmlspecialchars($app_name ?? 'Dashboard') ?></title>
...
<main class="pt-5 mt-4 mb-5">
  <div class="container mt-4">
    <h2 class="mb-2">Team Records</h2>

    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
      <div>
        <a href="add-team" class="btn btn-success">
          <i class="bi bi-plus-circle me-1"></i> Add Team
        </a>
      </div>
      <div class="input-group" style="max-width: 300px;">
        <input id="searchInput" type="search" class="form-control" placeholder="Search teams..." />
    ...
    <div class="card shadow-sm p-3">
      <div class="table-responsive">
        <table id="teamTable" class="table table-bordered table-hover align-middle">
          ...
          <?php if (count($teams) > 0): ?>
            <?php foreach ($teams as $index => $team): ?>
              <tr>
                ...
                <td><?= htmlspecialchars($team['league_name']) ?></td>
                <td><?= htmlspecialchars($team['country']) ?></td>
                <td>
                  <a href="edit-team?id=<?= $team['id'] ?>" class="btn btn-sm btn-primary"><i class="bi bi-pencil"></i></a>
                  <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal"
                          data-id="<?= $team['id'] ?>" data-name="<?= htmlspecialchars($team['name']) ?>">
          ...
          <?php else: ?>
            <tr>
              <td colspan="6" class="text-center text-muted py-4">No team records found.</td>
            </tr>
          <?php endif; ?>
          </tbody>
        </table>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <strong id="teamToDelete" class="text-dark"></strong>?
      </div>
...
<script>
  ...
  confirmModal.addEventListener('show.bs.modal', function (event) {
    ...
    document.getElementById('confirmDeleteBtn').href = `delete-team?id=${teamId}`;
